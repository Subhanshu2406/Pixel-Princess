...
// Global variables
static GameState currentState = STATE_SPLASH;         // GUI WALA
static int selectedOption = 0;                        // GUI WALA
static Model modelMario, modelPrincess, modelMonster; // GUI WALA
...
int no_of_monsters;
coordinates Mario_coordinates, Princess_coordinates; // Mario and princess coordinates defined under the structure coordinates line 18 pe define kar rakkha hai
    ...
    Mario_coordinates.z == Princess_coordinates.z)
{
    ...
    // Write to file: Mario wins
    FILE *file = fopen("game_result.txt", "w");
    if (file != NULL)
    {
        fprintf(file, "Mario wins! The Princess has been rescued.\n");
        fprintf(file, "It took %d rounds to win.\n", round_number);
        fclose(file);
    }
    else
    {
        printf("Error opening file to write game result.\n");
    }
...
for (int i = 0; i < no_of_monsters; i++)
{
        ...
        Monsters_coordinates[i].z == Mario_coordinates.z)
    {
        ...
        // Write to file: Mario loses
        FILE *file = fopen("game_result.txt", "w");
        if (file != NULL)
        {
            fprintf(file, "Monsters win! Mario was captured.\n");
            fprintf(file, "It took %d rounds for the monsters to win.\n", round_number);
            fclose(file);
        }
        else
        {
        ...
        Monsters_coordinates[i].z == Princess_coordinates.z)
    {
        ...
        // Write to file: Monsters win
        FILE *file = fopen("game_result.txt", "w");
        if (file != NULL)
        {
            fprintf(file, "Monsters win! The Princess was captured.\n");
            fprintf(file, "It took %d rounds for the monsters to win.\n", round_number);
            fclose(file);
        }
        else
...
// MOVE KARAO SABKO MOOOOOOOOOOOVE
void move_entity(coordinates *entity, coordinates target, Stack *visited, const char *entity_name)
{
